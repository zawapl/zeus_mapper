...
structs, and what's confirmed used vs. unused by the game. This is the up-to-date reference; fields
without an entry here, or called out below as unconfirmed, are still-open questions tracked in the
project's todo list, not settled facts.
...
the episode-select screen - confirmed in-game. `to_pak` leaves `colony_episodes_used` at `0`; no
evidence it needs to be written.
...
visible. Disambiguating the three pairs is parked pending further `.sav` investigation (needs either
an in-game observation that splits a pair, or `.sav`'s own tab-visibility representation to be found
and cross-referenced). The remaining 7 of the original 16 varying offsets still don't correlate with
any tab at all.
...
field just because they read as one uniform run today - a run being uniformly constant across every
adventure surveyed is evidence about *value*, not about where one field ends and the next begins;
don't infer sub-field boundaries, counts, or primitive types from a byte range simply reading
uniformly so far.
...
| `active`/`visible`                                    | `active` via `resolve_active(location, new_file_ver)` (see the "`sprite_id`/`sprite_size` investigation" note below); `visible` via `resolve_visible(location, new_file_ver)` - new-format locations read `WorldLocationData.visible` directly, inverted (`0` = visible, `4` = not yet visible); old-format locations always leave `visible` `0` (the same bug shape `active_new` has) and instead carry the real flag in `favour_new`, which is never a real favour percentage in old format (see `favour` below) - confirmed across 198 real old-format `ForeignCity`/`DistantCity`/`EnchantedPlace` records. `ParentCity` doesn't carry `visible` (always visible in-game); `Colony` doesn't carry either (always active and visible)                                                                                                                                                            |
| `military_strength`/`economy_strength`/`favour`       | `WorldLocationData.military_strength`/`economical_strength`/`favour_new` directly on new-format locations; old-format locations always read `favour_old` instead (see `resolve_favour`) - old-format `favour_new` is never a real percentage, only ever `0` or `4` (the `visible` flag - see above), confirmed across 228 real old-format `Colony`/`ForeignCity`/`DistantCity`/`EnchantedPlace` records with zero exceptions. `ParentCity` carries none of these three; `EnchantedPlace` carries only `military_strength`                                                                                                                                                                                                                      |
...
- `SettingsData.map_data` duplicates `map_data.last()` byte-for-byte; confirmed dead in-game, but
  `Adventure::to_pak` still reproduces it for round-trip fidelity.
- `basic_episode_data`/`real_episode_data`/`mythology` slots beyond an episode's real index are
  populated with real-looking, confirmed-unused data - not reproduced.
- Map-level `mythology` gods/pyramids are confirmed unused; settings-level is authoritative
