...
`SettingsData.field_13`/`field_14`/`colony_episode_goal_counts` hold the exact boundary: `field_13:
BoxedArray<u8, 234>` is still-unidentified bytes, byte-identical between file generations;
`field_14: u32` (also still-unidentified) sits between it and `colony_episode_goal_counts` and is
present only for `new_file_ver`. This was confirmed by diffing raw bytes directly
rather than computing the split from field-size totals alone, against a new-format adventure found
to hold the same underlying content as `The Odyssey.pak` (`version_2 == 5`): every byte from
`parent_event_counts` through the end of `bitmap` (35,892 bytes) was identical between them, and
they stayed identical for another 234 bytes past `bitmap`. Right at that point the new-format file
had 4 extra bytes; past them the two stayed aligned (`Odyssey[234 + n] == other[238 + n]`) for
...
| Field                                                 | Source                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
|-------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `location_type`                                       | `WorldLocationData.location_type`, matches `LocationType` (`Parent`/`Colony`/`Foreign`/`Distant`/`Enchanted`) exactly for every value observed                                                                                                                                                                                                                                                                                                                                                                                                         |
| `position`/`marker_variant`/`sprite_id`/`sprite_size` | `WorldMapElementData.x`/`y`/`variant`(`1`=city marker, `2`=label)/`sprite_id`/`sprite_width`/`sprite_height`                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `traded_goods`                                        | `TradedGoods::vec_from_data(selling, buying, trade_quantities)` - same sold/bought-resource-id-list-plus-quantity-table shape as `RealEpisodeData.city_resources_sold`/`bought`/`quantity`, indexed directly by resource id like that field                                                                                                                                                                                                                                                                                                            |
| `name`                                                | `WorldLocationData.custom_name`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `leader_name`                                         | `custom_leader_name` if non-empty (`EntityName::Custom`), else the numeric `leader_name` field as an unresolved id (`EntityName::TextId`) - on non-"extras" format files `custom_leader_name` is always empty and `leader_name` holds a small numeric id, presumably into one of the `.eng` text-table groups (not cross-referenced yet)                                                                                                                                                                                                               |
...
| `civilization`                                        | `WorldLocationData.civilization` (formerly the unnamed `variant` field) - the world-map sprite set ("nation skin") a location's marker uses, a different encoding from the adventure-level `Civilization`. Confirmed: `0` = `Greek`, `2` = `Persian`, `3` = `Centaur`, `5` = `Egyptian`, `8` = `Oceanid`, `9` = `Atlantean`; other raw values (`1`/`4`/`6`/`7`) are seen in other real adventures but not yet cross-referenced. Meaningless on the parent city's own entry (slot `0`), which still carries whatever raw value the file happens to have |
| `status`                                              | `WorldLocationData.attitude` (`LocationStatus`: `0` = `Ally`, `1` = `Rival`, `3` = `Colony`) - this location's diplomatic stance toward the player. `Colony` is also observed, unexplained, on the parent city's own entry                                                                                                                                                                                                                                                                                                                             |
| `active`/`visible`                                    | `WorldLocationData.active`/`visible`, both boolean - `active` directly (`0`/`1`), `visible` inverted (`0` = visible, `4` = not yet visible; only these two raw values observed)                                                                                                                                                                                                                                                                                                                                                                        |
| `military_strength`/`economy_strength`/`favour`       | `WorldLocationData.military_strength`/`economical_strength`/`favour` directly, except old-format non-`Parent` locations with a zero `favour` field, which read `old_format_favour` instead (see `WorldLocation::from_data`)                                                                                                                                                                                                                                                                                                                            |
| `pay_tribute`                                         | `WorldLocationData.tribute != 0`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `pay`/`receive`                                       | `(ResourceType, MonetaryAmount)` from `(tribute_pay_resource, tribute_pay_amount)`/`(tribute_rec_resource, tribute_rec_amount)` - the two resource fields are independent resource ids, resolved via `ResourceType`                                                                                                                                                                                                                                                                                                                                    |
...
Still unmodeled on `WorldLocationData`: ~372 bytes across six `unknown_*` blocks (`unknown_240`
and `unknown_341` flank the resolved `old_format_favour` byte between them - see `favour` above).
Still unmodeled on
`WorldMapElementData`: `sprite_id`'s sprite-sheet mapping, `data_a`/`data_d`, and `unknown_a`/
`unknown_b` (the bytes flanking `label_position`; see above and their own field comments).
